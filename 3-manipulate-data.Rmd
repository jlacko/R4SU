...
- `unique()` výčet unikántních hodnot vektoru 
...
Důležitý koncept představuje pajpa (pipe) `%>%`. Znamená posunutí operátoru na první argument následující funkce. Na první pohled složité, ale ve skutečnosti velmi funční. Do hovorového jazyka se dobře překládá jako "a pak...".

Pro vložení pajpy můžeme použít klávesovou zkratku Ctrl Shift M.
...
Při použití `spread()` často získáme sloupce s názvy, které nevyhovují – napřiklad obsahují diakritiku, nebo mezery (nebo, jako  v našem případě, obojí). Pro jejich přejmenování můžeme použít funkci `colnames()` do které přiřadíme očekávaný vektor názvů.
...
* [Data Trasnsformation](https://r4ds.had.co.nz/transform.html) kapitola R for Data Science  
